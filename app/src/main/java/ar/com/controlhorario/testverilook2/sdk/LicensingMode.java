...
public enum LicensingMode {
	...
	public static EnumSet<LicensingMode> getAvailable() {
		try {
			return NLicensing.nativeModuleOf().getTitle().contains("(Trial)") ? EnumSet.of(DIRECTLY, FROM_PC) : EnumSet.of(FROM_PC, FROM_FILE);
		} catch (Throwable e) {
	...
	public static LicensingMode getDefault() {
		try {
			return NLicensing.nativeModuleOf().getTitle().contains("(Trial)") ? DIRECTLY : FROM_FILE;
		} catch (Throwable e) {
			Log.e(TAG, "Exception", e);
			return FROM_FILE;
		}
